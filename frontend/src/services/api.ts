import type { SearchRequest, SearchResponse, ChatRequest, ChatSearchResponse } from '../types';
...
class ApiService {
  ...
  /**
   * Send chat message and get search results with AI response
   * @param chatRequest - The chat message and context
   * @returns Promise with chat search results
   */
  async chatSearch(chatRequest: ChatRequest): Promise<ChatSearchResponse> {
    try {
